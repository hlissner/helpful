...
Each is responsible for its own whitespace and heading. Each take four
    ...
    (unless buf
      ...
      (when (numberp helpful-max-buffers)
        (let* ((buffers (buffer-list))
               (helpful-bufs (seq-filter (lambda (it)
                                           (with-current-buffer it
                                             (eq major-mode 'helpful-mode)))
                                         buffers))
               ;; `buffer-list' seems to be ordered by most recently
               ;; visited first, so keep those.
               (excess-buffers (seq-drop helpful-bufs (1- helpful-max-buffers))))
...
(defun helpful--format-closure (sym form)
  ...
  (let (docstring)
    (cl-destructuring-bind (_keyword _env args . body) form
...
(defun helpful--format-alias (sym callable-p)
  (let ((obsolete-info (helpful--obsolete-info sym callable-p))
    ...
    (cond
     (obsolete-info
      (if-let* ((version (car (last obsolete-info))))
...
(defun helpful--format-properties (symbol)
  ...
  (let* ((syms-and-vals
          (seq-partition (and (symbolp symbol) (symbol-plist symbol)) 2))
         (syms-and-vals
...
source code to primitives."
  (let ((emacs-src-dir (read-directory-name "Path to Emacs source code: ")))
    ...
    (unless (equal (file-name-nondirectory (directory-file-name emacs-src-dir))
                   "src")
...
or disable if already enabled."
  (let ((should-edebug (not (helpful--edebug-p sym))))
    (cl-destructuring-bind (buf pos created) (helpful--definition sym t)
      (if (and buf pos)
          (progn
            (with-current-buffer buf
              (save-excursion
...
(defun helpful--show-callees (button)
  ...
  (let* ((buf (get-buffer-create "*helpful callees*"))
         ...
         (primitives (seq-filter (lambda (sym) (helpful--primitive-p sym t)) syms))
         (compounds (seq-remove (lambda (sym) (helpful--primitive-p sym t)) syms)))
...
blank line afterwards."
  (let* ((lines (split-string docstring "\\(\r\n\\|[\n\r]\\)"))
    ...
    (if (and (string-suffix-p "." first-line)
             (stringp second-line)
             (not (equal second-line "")))
        (string-join (cons first-line (cons "" (cdr lines)))
                     "\n")
      docstring)))
...
bound) or else highlight."
  (let* ((sym (intern sym-name)))
    (cond
     ...
     ((and (boundp sym) (string-suffix-p "variable " (downcase before-txt)))
      (helpful--button
       sym-name
       'helpful-describe-exactly-button
       'symbol sym
     ...
     ((and (fboundp sym)
           (or
            (string-prefix-p " command" (downcase after-txt))
            (string-suffix-p "command " (downcase before-txt))
            (string-suffix-p "function " (downcase before-txt))))
      (helpful--button
       sym-name
       'helpful-describe-exactly-button
       'symbol sym
    ...
    (let (result)
      (dolist (item (cdr keymap))
        (cond
         ...
         ((consp item)
          (mapc (lambda (it)
                  (push (list (vconcat (vector (car item)) (car it)) (cadr it))
                        result))
                (helpful--keymap-keys (cdr item))))
         ;; Dense keymaps are char-tables.
         ((char-table-p item)
          (map-char-table
           (lambda (keycode value)
             (mapc (lambda (it)
                     (push (list (vconcat (vector keycode) (car it)) (cadr it))
                           result))
                   (helpful--keymap-keys value)))
           item))))
      ...
      (setq result
            (mapcar
             (lambda (it)
               (list
                (where-is-internal (elt (car it) 1) global-map t)
                (cadr it)))
             (seq-filter
              (lambda (it)
                (and (> (length (car it)) 1)
                     (eq (elt (car it) 0) 'remap)))
              result)))
...
(defun helpful--format-keymap (keymap)
  ...
  (let* ((keys-and-commands (helpful--keymap-keys keymap))
         ...
         (keys (mapcar #'car keys-and-commands))
         ;; Add padding so all our strings are the same length.
         (formatted-keys (mapcar #'key-description keys))
         (max-formatted-length (apply #'max (cons 0 (mapcar #'length formatted-keys))))
         (aligned-keys (mapcar (lambda (it)
                                 (string-pad it (1+ max-formatted-length)))
                               formatted-keys))
         ;; Format commands as buttons.
         (commands (mapcar #'cadr keys-and-commands))
         (formatted-commands
          (mapcar
           (lambda (it)
             (cond
         ...
         (lines
          (mapcar (lambda (it) (format "%s %s" (car it) (cdr it)))
                  (seq-mapn #'cons aligned-keys formatted-commands))))
    ;; The flattened keymap will have normal bindings first, and
    ;; inherited bindings last. Sort so that we group by prefix.
    (string-join (seq-sort #'string< lines) "\n")))
  ...
  (catch 'source
    ...
    (let ((source nil))
      (when (and buf pos)
        ...
        (setq source (string-trim-right source))
...
If .elc files exist without the corresponding .el, return nil."
  (when (member (file-name-extension library-name) '("c" "rs"))
    (setq library-name
          (expand-file-name (concat "../" library-name)
  ...
  (let ((primitive-p (helpful--primitive-p sym callable-p))
    ...
    (cond
     ...
     ((and callable-p library-name)
      (when src-path
        (cl-destructuring-bind (src-buf src-opened)
            (helpful--open-if-needed src-path)
          (setq buf src-buf)
...
(defun helpful--reference-positions (sym callable-p buf)
  ...
  (let* ((forms-and-bufs
         ...
         (forms-and-buf (car forms-and-bufs))
         (forms (car forms-and-buf)))
    (mapcar
     (lambda (it) (nth 1 it))  ; (_code start-pos _end-pos)
     forms)))
...
(defun helpful--key-sequences (command-sym keymap global-keycodes)
  ...
  (let* ((keycodes
    ...
    (thread-last
      ...
      (seq-remove
       (lambda (it)
         (or (member it global-keycodes)
             (member it parent-keycodes))))
...
same bindings as `global-map'."
  (let* ((keymap-syms (helpful--all-keymap-syms))
    ...
    (mapc
     (lambda (it)
       (let* ((keymap-sym (car it))
              (keymap (cdr it))
              (key-sequences (helpful--key-sequences command-sym keymap global-keycodes)))
         (when (and key-sequences (not (eq keymap-sym 'widget-global-map)))
           (push (cons (symbol-name keymap-sym) key-sequences)
                 matching-keymaps))))
     (seq-mapn #'cons keymap-syms keymap-sym-vals))
...
alist with the lists concatenated."
  (let* ((l1-keys (mapcar #'car l1))
         (l2-keys (mapcar #'car l2))
         (l2-extra-keys (seq-difference l2-keys l1-keys))
         (l2-extra-values
          (mapcar (lambda (it) (assoc it l2)) l2-extra-keys))
         (l1-with-values
          (mapcar (lambda (it)
                    (cons (car it) (append (cdr it)
                                           (cdr (assoc (car it) l2)))))
                  l1)))
...
`helpful--keymaps-containing'."
  (let* ((syms (cons command-sym aliases))
         (syms-keymaps (mapcar #'helpful--keymaps-containing syms)))
    (seq-reduce #'helpful--merge-alists (cdr syms-keymaps) (car syms-keymaps))))

(defun helpful--format-keys (command-sym aliases)
  "Describe all the keys that call COMMAND-SYM."
  (let (mode-lines
        global-lines)
    (dolist (it (helpful--keymaps-containing-aliases command-sym aliases))
      (let ((map (car it))
            (keys (cdr it)))
        (dolist (key keys)
          (push
    ...
    (setq global-lines (seq-sort #'string< global-lines))
    (setq mode-lines (seq-sort #'string< mode-lines))
    (let ((lines (append global-lines mode-lines)))
      (if lines
          (string-join lines "\n")
...
Moves point in BUF."
  (with-current-buffer buf
    ...
    (list (point) (seq-take (read buf) 2))))
...
to its source location."
  (let ((formatted-count
    ...
    (propertize
     (format
      "%s %s"
      (string-pad (helpful--format-head head) longest-head)
...
POSITION-HEADS takes the form ((123 (defun foo)) (456 (defun bar)))."
  (let ((longest-head
         (thread-last position-heads
                      (mapcar (lambda (it) (helpful--format-head (cadr it))))
                      (mapcar #'length)
                      (apply #'max))))
    (string-join
     (mapcar (lambda (it)
...
(defun helpful--autoloaded-p (sym buf)
  ...
  (when-let* ((file-name (buffer-file-name buf)))
    (setq file-name (string-remove-suffix ".gz" file-name))
    (condition-case nil
         ...
         (buttons
          (list
           ...
           (if (and (not callable-p) (local-variable-if-set-p sym))
               buffer-local-button)))
         (description
          (helpful--join-and (remq nil buttons)))
         (kind
          (cond
           ((special-form-p sym)
            special-form-button)
...
(defun helpful--callees (form)
  ...
  (let* ((expanded-form (macroexpand-all form))
         ...
         (form-syms (seq-filter #'symbolp (flatten-list form)))
         (form-fns (seq-filter (lambda (it) (memq it form-syms)) all-fns)))
    (seq-uniq form-fns)))
...
may contain duplicates."
  (cond
   ...
   ((memq (car form) '(and catch defconst defvar if interactive
    ...
    (flatten-list
     (mapcar #'helpful--callees-1 (cdr form))))
   ...
   ((eq (car form) 'cond)
    (let* ((clauses (cdr form))
           (clause-fns
            ...
            (mapcar (lambda (it)
                      (mapcar #'helpful--callees-1 it))
                    clauses)))
      (flatten-list clause-fns)))

   ((eq (car form) 'condition-case)
    (let* ((protected-form (nth 2 form))
           (protected-form-fns (helpful--callees-1 protected-form))
           (handlers (seq-drop form 3))
           (handler-bodies (mapcar #'cdr handlers))
           (handler-fns
            (mapcar
             (lambda (it)
               (mapcar #'helpful--callees-1 it))
   ...
   ((eq (car form) 'lambda)
    ...
    (flatten-list (mapcar #'helpful--callees-1 (seq-drop form 2))))

   ((eq (car form) 'closure)
    ;; Same as lambda, but has an additional argument of the
    ;; closed-over variables.
    (flatten-list (mapcar #'helpful--callees-1 (seq-drop form 3))))

   ((memq (car form) '(let let*))
    ;; Extract function calls used to set the let-bound variables.
    (let* ((var-vals (cadr form))
           (var-val-callees
            (mapcar
             (lambda (it)
               (if (consp it)
             ...
             var-vals)))
      (append
       (flatten-list var-val-callees)
       ;; Function calls in the let body.
       (mapcar #'helpful--callees-1 (seq-drop form 2)))))
...
state of the current symbol."
  ...
  (let ((gc-cons-threshold most-positive-fixnum)
    ...
    (let* ((val
      ...
      (run-hook-with-args 'helpful-update-functions
                          helpful--sym
                          helpful--callable-p
                          :value val
                          :primitive-p primitive-p
                          :associated-buffer helpful--associated-buffer
                          :buffer buf
                          :position pos
                          :opened-p opened
                          :source source
...
(defun helpful--format-symbol (sym)
  ...
  (replace-regexp-in-string " " "\\ " (format "%s" sym) t t))
  ...
  (let (docstring-sig
    ...
    (let* ((function-args
           ...
           (formatted-args
            (cond
             (advertised-args
              (mapcar #'helpful--format-argument advertised-args))
             ((listp function-args)
              (mapcar #'helpful--format-argument function-args))
             (t
              (list function-args)))))
      (setq source-sig
            (cond
...
(defun helpful--format-obsolete-info (sym callable-p)
  (cl-destructuring-bind (use _ date) (helpful--obsolete-info sym callable-p)
    ...
    (when (string-prefix-p prefix string)
      (intern
       (string-remove-prefix
        prefix
        (replace-regexp-in-string "_" "-" string t t))))))
...
(defun helpful--support-storing-org-links ()
  ...
  (when (and (fboundp 'org-link-set-parameters)
             (not (member "helpful" (org-link-types))))
...
(defun helpful-insert-summary (symbol callable? &rest plist)
  "Insert a summary for SYMBOL, plus any obsolescence info about it.

For `helpful-update-functionsal'."
  (insert (helpful--summary
           symbol callable?
           (plist-get plist :buffer)
           (plist-get plist :position)))
  (when (helpful--obsolete-info symbol callable?)
    (helpful--insert-section-break)
    (insert (helpful--format-obsolete-info symbol callable?))))

(defun helpful-insert-signature (symbol callable? &rest _plist)
  "Insert the signature for SYMBOL if it is callable.

For `helpful-update-functionsal'."
  (when (and callable? (not (helpful--kbd-macro-p symbol)))
    (helpful--insert-section-break)
    (insert (helpful--heading "Signature")
            (helpful--syntax-highlight (helpful--signature symbol)))))

(cl-defun helpful-insert-values (symbol callable? &key
                                        value
                                        position
                                        associated-buffer
                                        &allow-other-keys)
  "Insert the values for SYMBOL, assuming it's not callable.

For `helpful-update-functionsal'."
  (unless callable?
    (helpful--insert-section-break)
    (let ((multiple-views-p
           (or (stringp value)
               (keymapp value)
      ...
      (insert (helpful--heading
               (cond
                ;; Buffer-local variable and we're looking at the value in a specific
                ;; buffer.
                ((and associated-buffer (local-variable-p symbol associated-buffer))
                 (format "Value in %s"
                         (helpful--button
                          (format "#<buffer %s>" (buffer-name associated-buffer))
                          'helpful-buffer-button
                          'buffer associated-buffer
                          'position position)))
                ;; Buffer-local variable but default/global value.
                ((local-variable-if-set-p symbol) "Global Value")
                ;; This variable is not buffer-local.
                (t "Value")))
              (helpful--format-value symbol value)
              "\n\n")
      (when (helpful--original-value-differs-p symbol)
        (insert (helpful--heading "Original Value")
                (helpful--format-value
                 symbol
                 (car (helpful--original-value symbol)))
                "\n\n"))
      ...
      (when (local-variable-if-set-p symbol)
        (insert (helpful--button
                ...
                (helpful--button
                 ...
                 'prompt-p nil)
                " "))
      (when (memq (helpful--sym-value symbol associated-buffer) '(nil t))
        (insert (helpful--make-toggle-button symbol associated-buffer) " "))
      (insert (helpful--make-set-button symbol associated-buffer))
      (when (custom-variable-p symbol)
        (insert " " (helpful--make-customize-button symbol))))))

(defun helpful-insert-docs (symbol callable? &rest _plist)
...
For `helpful-update-functions'."
  (when (commandp symbol)
    (helpful--insert-section-break)
    (insert (helpful--heading "Key Bindings")
            (helpful--format-keys symbol (plist-get plist :aliases)))))

(cl-defun helpful-insert-references (symbol callable? &key
                                            position
                                            primitive-p
                                            source
                                            source-path
                                            &allow-other-keys)
  "Insert a list of references to SYMBOL.

For `helpful-update-functions'."
  (let* ((references (helpful--calculate-references
                      symbol callable? source-path)))
    (helpful--insert-section-break)
    (insert (helpful--heading "References")
            (let ((src-button
                   (when source-path
                     (helpful--navigate-button
            ...
            "\n\n"
            (helpful--make-references-button symbol callable?))
    (when (and callable?
               (symbolp symbol)
               source
               (not primitive-p))
  ...
  (let ((can-edebug
        ...
        (can-trace
         (and (symbolp symbol)
              ...
              (not (plist-get (symbol-plist symbol) 'byte-opcode))))
        (can-disassemble
         (and callable? (not primitive-p)))
        (can-forget
         (and (not (special-form-p symbol))
              (not primitive-p))))
    (when (or can-edebug can-trace can-disassemble can-forget)
      (helpful--insert-section-break)
      (insert (helpful--heading "Debugging")))
    (when can-edebug
      (insert (helpful--make-edebug-button symbol)))
    (when can-trace
      (when can-edebug
        (insert " "))
      (insert (helpful--make-tracing-button symbol)))

    (when (and (or can-edebug can-trace)
               (or can-disassemble can-forget))
      (insert "\n"))

    (when can-disassemble
      (insert (helpful--make-disassemble-button symbol)))
...
For `helpful-update-functions'."
  (when-let* ((aliases (plist-get plist :aliases)))
    ...
    (insert (helpful--heading "Aliases")
            (string-join (mapcar (lambda (it) (helpful--format-alias it callable?))
                                 aliases)
                         "\n"))))
    ...
    (let (content)
      (when (fboundp #'cl--generic-describe)
        (with-temp-buffer
          ...
          (cl--generic-describe symbol)
          (goto-char (point-min))
          (when (re-search-forward "^Implementations:$" nil t)
            (setq content (buffer-substring (point) (point-max)))))
        (when content
          (helpful--insert-section-break)
          (insert (helpful--heading "Implementations") (string-trim content)))))))

(cl-defun helpful-insert-source-code (symbol callable? &key
                                             source
                                             source-path
                                             primitive-p
                                             position
...
;;; ** Populate `helpful-update-functions'

(add-hook 'helpful-update-functions #'helpful-insert-summary 5)
...
(add-hook 'helpful-update-functions #'helpful-insert-references 50)
(add-hook 'helpful-update-functions #'helpful-insert-implementations 60)
(add-hook 'helpful-update-functions #'helpful-insert-debugging-buttons 70)
(add-hook 'helpful-update-functions #'helpful-insert-source-code 80)
(add-hook 'helpful-update-functions #'helpful-insert-symbol-properties 90)
